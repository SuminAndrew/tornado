...
from tornado.routing import (ANY_MATCHES, default_host_matches, host_matches,
...
class Application(ReversibleRouter):
    ...
    def __init__(self, handlers=None, default_host=None, transforms=None,
        ...
        self.default_router.add_handlers([
            Rule(ANY_MATCHES, self.wildcard_router)
        ])
    ...
    def add_handlers(self, host_pattern, host_handlers):
        """Appends the given handlers to the handler list of a default router.

        For more details see ``tornado.web.RegexRouter.add_handlers``.
        """
        host_matcher = host_matches(host_pattern)
        rule = Rule(host_matcher, ApplicationRouter(self, host_handlers))

        self.default_router.rules.insert(-1, rule)

        if self.default_host is not None:
            self.wildcard_router.add_handlers([(
                default_host_matches(self, host_matcher.host_pattern),
                host_handlers
            )])
    ...
    def reverse_url(self, name, *args):
        ...
        In default router implementation the handler must be added to the
        application as a named `URLSpec`.
        For more details see ``tornado.web.RegexRouter.reverse_url``.

        Other router implementations may have their own `reverse_url` logic.
        """
